...
3. 在虚拟环境中安装连接MySQL数据库所需的依赖项。
   ...
   > 
   > 注意！： mac 直接安装 mysqlclient会失败，需要先安装mysql ,具体可查看
   >https://www.cnblogs.com/goldsunshine/p/15644298.html
   > 
   > ```Python
   > import pymysql
   > 
...
# 数据更新
Teacher.objects.filter(pk=1).update(name="李四")
```
...
```Shell
# 查询编号为1的学科的老师
Teacher.objects.filter(subject__no=1)  # 这里的 subject 是类中外键属性值
Subject.objects.get(pk=1).teacher_set.all() 

# 查询学科名称有“全栈”二字的学科的老师
Teacher.objects.filter(subject__name__contains='全栈')   # 这里的 subject 是类中外键属性值
```
